...
Base of 20(Ancient) Classical Base is 50(250% more than Ancient),  Medieval is 150(300% more than Classical), Renaissance is 450(300% more than Medieval), Industrial is 1575(300% more than Renaissance), 
Modern is 5512.5(300% more than Industrial), Digital is 19293.75(350% more than Modern), Information is 77175(350% more than Digital).
...
--Future Tier 5
UPDATE Technologies
SET Cost = Round(49612.5 * 1.75, 0) WHERE Tier = 5 AND Era = 'ERA_FUTURE';
--Future Tier 4
UPDATE Technologies
SET Cost = Round(49612.5 * 1.6, 0) WHERE Tier = 4 AND Era = 'ERA_FUTURE';
--Future Tier 3
UPDATE Technologies
SET Cost = Round(49612.5 * 1.4, 0) WHERE Tier = 3 AND Era = 'ERA_FUTURE';
--Future Tier 2
UPDATE Technologies
SET Cost = Round(49612.5 * 1.2, 0) WHERE Tier = 2 AND Era = 'ERA_FUTURE';
--Future Tier 1
UPDATE Technologies
SET Cost = Round(49612.5 * 1.0, 0) WHERE Tier = 1 AND Era = 'ERA_FUTURE';
...
--Industrial Tier 4
UPDATE Technologies
SET Cost = Round(1350 * 1.6, 0) WHERE Tier = 4 AND Era = 'ERA_INDUSTRIAL';
--Industrial Tier 3
UPDATE Technologies
SET Cost = Round(1350 * 1.4, 0) WHERE Tier = 3 AND Era = 'ERA_INDUSTRIAL';
--Industrial Tier 2
UPDATE Technologies
SET Cost = Round(1350 * 1.2, 0) WHERE Tier = 2 AND Era = 'ERA_INDUSTRIAL';
--Industrial Tier 1
UPDATE Technologies
SET Cost = Round(1350 * 1.0, 0) WHERE Tier = 1 AND Era = 'ERA_INDUSTRIAL';
...
INSERT INTO GameSpeeds		(ID, Type,					Description,					Help,								DealDuration,	GrowthPercent,	TrainPercent,	ConstructPercent,	CreatePercent,	ResearchPercent,	GoldPercent,	GoldGiftMod,	BuildPercent,	ImprovementPercent, GreatPeoplePercent, CulturePercent, FaithPercent,	BarbPercent,	FeatureProductionPercent,	UnitDiscoverPercent,	UnitHurryPercent,	UnitTradePercent,	GoldenAgePercent,	HurryPercent,	InflationPercent,	InflationOffset,	ReligiousPressureAdjacentCity,	VictoryDelayPercent,	MinorCivElectionFreqMod,	OpinionDurationPercent, SpyRatePercent, PortraitIndex,	IconAtlas)
				 ...
				 SELECT		 2,	 'GAMESPEED_STANDARD',	'TXT_KEY_GAMESPEED_STANDARD',	'TXT_KEY_GAMESPEED_STANDARD_HELP',	30,				115,			90,				80,					85,				95,				    95,				100,	        100,			100,				100,				120,			120,			100,			100,						125,					95,					100,				100,				95,				20,					-112,				6,								100,					100,						100,					100,			2,				'GAMESPEED_ATLAS'		UNION ALL
