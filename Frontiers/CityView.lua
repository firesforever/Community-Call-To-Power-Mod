...
function CityScreenClosed()
	...
	CloseAdvisorTutorialPopup(TutorialButtonPopupTypes.TUTORIALBUTTONPOPUP_CITYSCREEN);
		...
		if numSpecialBuildingsInThisCity > 0 then
			...
			if specialistBuildingHeadingOpen then
				...
				for i, v in ipairs(sortedList) do
					...
					local Hidebuilding = building.HiddenBuilding
--					print (Hidebuilding)
					if not (Hidebuilding ==1) then
...
function OnEnterCityScreen()
	...
	OpenAdvisorTutorialPopup(TutorialButtonPopupTypes.TUTORIALBUTTONPOPUP_CITYSCREEN);
